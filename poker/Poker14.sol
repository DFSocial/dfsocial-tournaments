...
contract Poker14 is Ownable {
    using SafeMath for uint;
    using EnumerableSet for EnumerableSet.AddressSet;
    
    /* Constants and Mains */
    uint constant tournament_type = 1 ;
    string public constant tournament_name = 'Poker #14';
    string public constant start_date = '12th Dec - 16:00 UTC';
    uint public prizepool = 1250000000000000000000 ;
    uint256[] public prizepools_up = [2500000000000000000000,5000000000000000000000];
    
    uint public constant timeLocked = 30 days;
    uint public constant timePaying = 5 days;
    uint public constant amountToLock = 500000000000000000000;
    uint public pointsPerRegistration = 6;
    
    uint public constant percentage = 45;
    uint public constant playersPerTeam = 1;
    uint public maxParticipants = 250;
    uint public numParticipants;
    
    /* UNIX date when register finish: 11th Dec 23:59 UTC */
    uint public register_date = 1639267140; 
    ...
    bool public paused;
    uint256[] extraPoints = [25,20,15,10,5];
    ...
    /*
    function getSponsored(address _sponsor, uint _index) public view returns (teamSponsored memory) {
        require(_index < data[_sponsor].numSponsored);
        return sponsored[_sponsor][_index];
    }
    
    function getSponsorData(address _sponsor) public view returns (sponsor_data memory){
    ...
    */
    ...
    function addParticipant(address _sponsor, address[] calldata _players, string memory _name, bool _invite) public returns (bool){
        ...
        if(_invite){
            ...
            require(Token(tokenAddress).transferFrom(_sponsor, address(this) , amountToLock ) , "Error locking tokens");     
        ...
        require(Token(sponsorLeague).addPoints(_sponsor, pointsPerRegistration), "Sponsor League error");
        ...
        if(numGamers > 31 && numGamers < 64){
            prizepool = prizepools_up[0];
        }else if(numGamers > 63){
            prizepool = prizepools_up[1];
        }
    ...
    function setScore(address _sponsor, uint _index, uint _score, uint _pos) public onlyOperator returns(bool){
        ...
        if(_pos >= 1 && _pos <= 5){
             ...
             require(Token(sponsorLeague).addPoints(_sponsor, _extra), "Sponsor League error");
        ...
        for(uint i = 1; i <= numSponsors; i = i.add(1)){
            ...
            for(uint j = 0; j < getNumSponsored(_current); j = j.add(1)){
                ...
                if(team_score[aux_index][j] > 0){
                    ...
                    if(_reward > 0){
                        _rewHolder = _reward.mul(percentage).div(100);
                        _rewHolder = _rewHolder.mul(data[aux_index].multiplierSponsor).div(100);
                        _rewGamer = _reward.mul(percentage.div(playersPerTeam)).div(100);
                        _rewGamer = _rewGamer.mul(data[aux_index].multiplierGamer).div(100);
                        _extraHolder = _rewHolder.sub(_reward.mul(percentage).div(100));
                        _extraGamer = _rewGamer.sub(_reward.mul(percentage.div(playersPerTeam)).div(100));
                        ...
                        if(fee > _extraHolder.add(_extraGamer)){
                         ...
                         if(_extraHolder > 0){
                            ...
                            require(Token(Collection).gotBonus(_current,  nft_id, _extraTotal), "Error adding bonus");
                        }
                        ...
                        require(Token(tokenAddress).transfer(referee, fee), "Error sending fee");
                        require(Token(tokenAddress).transfer(_current, _rewHolder), "Error sending to sponsor");
                        for(uint k = 0; k < playersPerTeam; k = k.add(1)){
                            auxGamer = sponsored[aux_index][j].players[k];
                            require(Token(tokenAddress).transfer(auxGamer, _rewGamer), "Error adding to gamer");
    ...
    function unlockTeamPaying(address _dir, uint _index) public returns (bool){
        ...
        require (Token(tokenAddress).transfer(referee, fee ) );
        require (Token(tokenAddress).transfer(_dir, _send ) );
        totalLockedAmount = totalLockedAmount.sub(amountToLock);
    ...
    function changeNamePaying(string memory _new, address _sponsor, uint _index) public returns (bool){
        ...
        if(msg.sender != referee){
            ...
            require (Token(tokenAddress).transferFrom(_sponsor, address(this), nameFee));
        }
    ...
    function recoverTokens(address _to, uint _amount) public onlyReferee returns (bool){
        require(Token(tokenAddress).balanceOf(address(this)).sub(_amount) >= totalLockedAmount, "User funds protector");
